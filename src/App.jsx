...
const [step, setStep] = useState(0); // 0=role, 1=pain, 2=tools, 3=switch, 4=email, 5=done
...
if (step === 1) return pains.length > 0;
if (step === 2) return tools.length > 0;
if (step === 3) return switchReasons.length > 0;
if (step === 4) return email.length > 3;
return false;
  };
const features = [
...
// ── Survey Chip Component ──
const Chip = ({ label, selected, onClick }) => (
<button onClick={onClick} style={{
padding: "14px 20px", borderRadius: 14, border: selected ? "2px solid #D97706" : "2px solid #E2E8F0",
background: selected ? "#FFFBEB" : "#fff", color: selected ? "#92400E" : "#475569",
fontSize: 15, fontWeight: 700, cursor: "pointer", transition: "all 0.15s",
display: "flex", alignItems: "center", gap: 10, width: "100%", textAlign: "left",
boxShadow: selected ? "0 4px 16px rgba(245,158,11,0.12)" : "0 1px 4px rgba(0,0,0,0.04)",
    }}>
<span style={{
width: 24, height: 24, borderRadius: 7, flexShrink: 0,
border: selected ? "2px solid #D97706" : "2px solid #CBD5E1",
background: selected ? "#F59E0B" : "#fff",
display: "flex", alignItems: "center", justifyContent: "center",
      }}>
{selected && <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="#fff" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
</span>
{label}
</button>
  );
// ── Survey Step Renderer ──
const renderStep = () => {
if (step === 0) {
return (
<div>
<div style={{ fontSize: 14, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8 }}>Step 1 of 4</div>
<h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 26, fontWeight: 900, color: "#0F172A", marginBottom: 6 }}>I am a...</h3>
<p style={{ fontSize: 15, color: "#64748B", fontWeight: 500, marginBottom: 20 }}>This helps us tailor SparkBid to your needs.</p>
<div style={{ display: "flex", gap: 14 }}>
{[
              { id: "gc", emoji: "🏗️", title: "General Contractor", sub: "I need electrical subs" },
              { id: "ec", emoji: "⚡", title: "Electrical Contractor", sub: "I want to find projects" },
].map(r => (
<div key={r.id} onClick={() => setRole(r.id)} style={{
flex: 1, padding: "22px 18px", borderRadius: 16, cursor: "pointer", textAlign: "center",
border: role === r.id ? "2px solid #D97706" : "2px solid #E2E8F0",
background: role === r.id ? "#FFFBEB" : "#fff",
boxShadow: role === r.id ? "0 6px 20px rgba(245,158,11,0.12)" : "0 1px 4px rgba(0,0,0,0.04)",
transition: "all 0.15s",
              }}>
<div style={{ fontSize: 36, marginBottom: 8 }}>{r.emoji}</div>
<div style={{ fontSize: 16, fontWeight: 800, color: "#0F172A", marginBottom: 4 }}>{r.title}</div>
<div style={{ fontSize: 13, color: "#64748B", fontWeight: 500 }}>{r.sub}</div>
</div>
            ))}
</div>
</div>
...
if (step === 1) {
const painOptions = role === "ec" ? [
"Finding new projects to bid on",
"Estimating costs accurately",
"Writing professional proposals",
"Competing against lowball bids",
"Getting paid on time after the job",
"Keeping track of multiple bids",
"Not hearing back from GCs",
] : [
"Finding reliable electrical subs",
"Getting enough bids to compare",
"Bids coming in over budget",
"Verifying contractor licenses & insurance",
...
return (
<div>
<div style={{ fontSize: 14, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8 }}>Step 2 of 4</div>
<h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 26, fontWeight: 900, color: "#0F172A", marginBottom: 6 }}>
            What are your biggest headaches?
</h3>
<p style={{ fontSize: 15, color: "#64748B", fontWeight: 500, marginBottom: 20 }}>Pick all that apply — be honest, this shapes what we build.</p>
<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
{painOptions.map(p => (
<Chip key={p} label={p} selected={pains.includes(p)} onClick={() => toggleItem(pains, setPains, p)} />
            ))}
</div>
</div>
      );
...
if (step === 2) {
const toolOptions = role === "ec" ? [
"Word of mouth / relationships",
"Phone calls & texting GCs directly",
"Facebook groups",
"PlanHub",
"BuildOps / Procore",
"Craigslist / job boards",
"Local trade associations (NECA, IBEW)",
"I don't actively look — work finds me",
] : [
"My existing network of subs",
...
"Bidding platforms (other)",
];
return (
<div>
<div style={{ fontSize: 14, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8 }}>Step 3 of 4</div>
<h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 26, fontWeight: 900, color: "#0F172A", marginBottom: 6 }}>
            How do you {role === "ec" ? "find work" : "find electrical subs"} today?
</h3>
<p style={{ fontSize: 15, color: "#64748B", fontWeight: 500, marginBottom: 20 }}>Pick all that you use — even if just occasionally.</p>
<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
{toolOptions.map(t => (
<Chip key={t} label={t} selected={tools.includes(t)} onClick={() => toggleItem(tools, setTools, t)} />
            ))}
</div>
</div>
      );
...
if (step === 3) {
const switchOptions = role === "ec" ? [
"More project leads in my area",
"AI that writes my bid proposals for me",
"Knowing what other bids are coming in at",
"Getting paid faster through the platform",
"Free to use with no monthly fees",
"Seeing GC ratings before I bid",
"Simple app that doesn't waste my time",
] : [
"More qualified electrical bids per project",
"AI that helps compare bids fairly",
"Verified contractor licenses & insurance",
"Faster turnaround on getting bids back",
"Free to post projects",
"Seeing contractor ratings & past work",
"One place to manage all my electrical bidding",
];
return (
<div>
<div style={{ fontSize: 14, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8 }}>Step 4 of 4</div>
<h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 26, fontWeight: 900, color: "#0F172A", marginBottom: 6 }}>
            What would make you try SparkBid?
</h3>
<p style={{ fontSize: 15, color: "#64748B", fontWeight: 500, marginBottom: 20 }}>Pick the features that matter most to you.</p>
<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
{switchOptions.map(s => (
<Chip key={s} label={s} selected={switchReasons.includes(s)} onClick={() => toggleItem(switchReasons, setSwitchReasons, s)} />
            ))}
</div>
</div>
      );
...
return (
<div>
<div style={{ fontSize: 14, fontWeight: 700, color: "#16A34A", textTransform: "uppercase", letterSpacing: 2, marginBottom: 8 }}>Almost done!</div>
<h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 26, fontWeight: 900, color: "#0F172A", marginBottom: 6 }}>
            Join the waitlist
</h3>
<p style={{ fontSize: 15, color: "#64748B", fontWeight: 500, marginBottom: 24 }}>
            Your answers help us build exactly what you need. Early members get <strong style={{ color: "#D97706" }}>6 months free</strong> when we launch.
</p>
<div style={{ marginBottom: 16 }}>
<label style={{ fontSize: 14, fontWeight: 700, color: "#1B2537", marginBottom: 8, display: "block" }}>Email address *</label>
<input
...
<div style={{ marginBottom: 16 }}>
<label style={{ fontSize: 14, fontWeight: 700, color: "#1B2537", marginBottom: 8, display: "block" }}>Company name <span style={{ color: "#94A3B8", fontWeight: 500 }}>(optional)</span></label>
<input
type="text" placeholder="e.g., Thompson Electric" value={companyName}
...
<p style={{ fontSize: 16, color: "#64748B", fontWeight: 500, lineHeight: 1.6, marginBottom: 8 }}>
          Thanks for sharing your insights{companyName ? `, ${companyName}` : ""}. We'll email you at <strong style={{ color: "#0F172A" }}>{email}</strong> when SparkBid launches.
</p>
<p style={{ fontSize: 15, color: "#64748B", fontWeight: 500, marginBottom: 24 }}>
          Your feedback on {pains.length} pain points and {switchReasons.length} must-have features is already shaping what we build.
</p>
<div style={{ padding: "16px 20px", borderRadius: 14, background: "#FEF3C7", fontSize: 15, fontWeight: 600, color: "#92400E" }}>
...
return (
<div style={{ fontFamily: "'DM Sans', sans-serif", color: "#1B2537", background: "#FAFBFC", overflowX: "hidden" }}>
<style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes sparkFloat { 0%,100% { transform:translateY(0) rotate(0deg); } 50% { transform:translateY(-12px) rotate(3deg); } }
        ...
        .cta-btn { display:inline-flex; align-items:center; gap:10px; padding:18px 36px; border-radius:14px; border:none; font-family:'DM Sans',sans-serif; font-size:17px; font-weight:700; cursor:pointer; transition:all 0.25s ease; text-decoration:none; }
        .cta-primary { background:linear-gradient(135deg,#F59E0B,#D97706); color:#1B2537; box-shadow:0 4px 24px rgba(245,158,11,0.35); }
        .cta-primary:hover { transform:translateY(-2px); box-shadow:0 8px 32px rgba(245,158,11,0.45); }
        .cta-secondary { background:#fff; color:#1B2537; border:2px solid #D1D9E0; box-shadow:0 2px 12px rgba(0,0,0,0.04); }
        .cta-secondary:hover { border-color:#F59E0B; transform:translateY(-2px); }
        .feature-card { padding:32px; border-radius:20px; background:#fff; border:1px solid #E8EDF2; transition:all 0.3s ease; box-shadow:0 2px 12px rgba(0,0,0,0.03); }
        ...
        .nav-link { font-size:15px; font-weight:600; color:#64748B; text-decoration:none; transition:color 0.2s; cursor:pointer; background:none; border:none; font-family:inherit; }
...
<div style={{ width: 38, height: 38, borderRadius: 10, background: "linear-gradient(135deg, #F59E0B, #D97706)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 12px rgba(245,158,11,0.3)" }}>
...
<button className="cta-primary cta-btn" style={{ padding: "12px 24px", fontSize: 15 }} onClick={scrollToSignup}>
              Join the Waitlist
</button>
</div>
...
{mobileMenuOpen ? <path strokeLinecap="round" d="M18 6L6 18M6 6l12 12" /> : <path strokeLinecap="round" d="M4 6h16M4 12h16M4 18h16" />}
</svg>
</button>
</div>
{mobileMenuOpen && (
...
<button className="cta-primary cta-btn" style={{ padding: "12px 28px", fontSize: 15 }} onClick={() => { setMobileMenuOpen(false); scrollToSignup(); }}>Join the Waitlist</button>
</div>
        )}
</nav>
{/* ═══════ HERO ═══════ */}
<section style={{ paddingTop: 120, paddingBottom: 80, position: "relative", overflow: "hidden" }}>
<div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundImage: "radial-gradient(circle at 1px 1px, #E2E8F0 1px, transparent 0)", backgroundSize: "40px 40px", opacity: 0.5 }} />
<div style={{ position: "absolute", top: -100, right: -200, width: 600, height: 600, background: "radial-gradient(circle, rgba(245,158,11,0.08) 0%, transparent 70%)", borderRadius: "50%" }} />
...
{animateIn && (<>
<div className="fade-up" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 16px", borderRadius: 50, background: "#FEF3C7", marginBottom: 24, fontSize: 14, fontWeight: 700, color: "#92400E" }}>
<span style={{ width: 8, height: 8, borderRadius: "50%", background: "#F59E0B", animation: "dotPulse 2s infinite" }} />
                Coming Soon — Join the Waitlist
</div>
<h1 className="fade-up-d1" style={{ fontFamily: "'Fraunces', serif", fontSize: 54, fontWeight: 900, lineHeight: 1.1, letterSpacing: -1.5, color: "#0F172A", marginBottom: 24 }}>
                Win more electrical<br />work. <span style={{ color: "#D97706" }}>Bid smarter.</span>
</h1>
<p className="fade-up-d2" style={{ fontSize: 19, lineHeight: 1.7, color: "#475569", fontWeight: 500, marginBottom: 36, maxWidth: 520 }}>
...
<button className="cta-btn cta-primary" onClick={scrollToSignup}>
                  Get Early Access
...
{[{ icon: "✓", text: "Free to join" }, { icon: "✓", text: "AI-powered bids" }, { icon: "✓", text: "No credit card needed" }].map((item, i) => (
...
{animateIn && (
<div className="fade-up-d2 spark-float" style={{ width: 320, background: "#fff", borderRadius: 28, overflow: "hidden", boxShadow: "0 24px 80px rgba(0,0,0,0.12), 0 0 0 1px rgba(0,0,0,0.04)" }}>
...
<div><div style={{ fontSize: 10, color: "#B45309", fontWeight: 700 }}>RECOMMENDED</div><div style={{ fontSize: 20, fontWeight: 900, color: "#16A34A" }}>$51,750</div></div>
<div style={{ textAlign: "right" }}><div style={{ fontSize: 10, color: "#B45309", fontWeight: 700 }}>WIN CHANCE</div><div style={{ fontSize: 20, fontWeight: 900, color: "#2563EB" }}>68%</div></div>
</div>
</div>
</div>
            )}
...
<p style={{ fontSize: 18, color: "#64748B", fontWeight: 500, maxWidth: 560, margin: "0 auto" }}>No complicated software. No steep learning curves. Just a simple, powerful tool that gets you more work.</p>
</div>
...
<h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 40, fontWeight: 900, color: "#0F172A", letterSpacing: -0.5 }}>Simple for everyone</h2>
</div>
<div className="two-col" style={{ display: "flex", gap: 60 }}>
<div style={{ flex: 1 }}>
<div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 18px", borderRadius: 50, background: "#EBF5FF", marginBottom: 28, fontSize: 14, fontWeight: 700, color: "#1D4ED8" }}>🏗️ For General Contractors</div>
{[
...
<div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: "#1D4ED8", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Fraunces', serif", fontSize: 20, fontWeight: 900 }}>{s.step}</div>
<div>
...
<div style={{ flex: 1 }}>
<div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 18px", borderRadius: 50, background: "#FEF3C7", marginBottom: 28, fontSize: 14, fontWeight: 700, color: "#92400E" }}>⚡ For Electrical Contractors</div>
{[
...
<div key={i} style={{ display: "flex", gap: 18, marginBottom: 28 }}>
<div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: "#D97706", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Fraunces', serif", fontSize: 20, fontWeight: 900 }}>{s.step}</div>
<div>
<div style={{ fontSize: 18, fontWeight: 800, color: "#0F172A", marginBottom: 4 }}>{s.title}</div>
<div style={{ fontSize: 15, color: "#64748B", lineHeight: 1.6, fontWeight: 500 }}>{s.desc}</div>
</div>
...
<blockquote style={{ fontFamily: "'Fraunces', serif", fontSize: 26, fontWeight: 700, color: "#0F172A", lineHeight: 1.5, marginBottom: 24, fontStyle: "italic" }}>
...
background: i <= step ? "linear-gradient(90deg, #F59E0B, #D97706)" : "#E2E8F0",
transition: "all 0.3s ease",
                  }} />
                ))}
</div>
<div style={{ fontSize: 12, fontWeight: 600, color: "#94A3B8", marginTop: 8, textAlign: "right" }}>
{step < 4 ? `${Math.round(((step + 1) / 5) * 100)}% complete` : "Final step!"}
</div>
</div>
          )}
{renderStep()}
...
padding: "16px 24px", borderRadius: 12, border: "2px solid #E2E8F0",
...
background: canAdvance() ? "linear-gradient(135deg, #F59E0B, #D97706)" : "#E2E8F0",
color: canAdvance() ? "#1B2537" : "#94A3B8",
fontSize: 16, fontWeight: 800, cursor: canAdvance() ? "pointer" : "not-allowed",
fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s",
boxShadow: canAdvance() ? "0 4px 16px rgba(245,158,11,0.3)" : "none",
display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                }}
>
{step === 4 ? "Join the Waitlist" : "Continue"}
<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
</button>
</div>
          )}
{/* Privacy note */}
{step < 5 && (
<div style={{ textAlign: "center", marginTop: 16, fontSize: 12, color: "#94A3B8", fontWeight: 500 }}>
              Your answers are confidential and only used to build a better product for you.
</div>
          )}
</div>
</section>
...
onClick={() => { const ids = ["features", "how-it-works", "signup"]; document.getElementById(ids[i])?.scrollIntoView({ behavior: "smooth" }); }}>
{item}
</button>
              ))}
</div>
...
<div style={{ fontSize: 13, color: "#475569", fontWeight: 500 }}>© 2026 SparkBid. All rights reserved.</div>
<div style={{ fontSize: 13, color: "#475569", fontWeight: 500 }}>The AI-powered marketplace for electrical contractor bidding.</div>
</div>
</div>
</footer>
</div>
